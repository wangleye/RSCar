...
for tr,tst in kf:
    trk = np.asarray(data.keys())[tr]
    tstk = np.asarray(data.keys())[tst]
    train_data = dict((k,v) for k,v in data.iteritems() if k in trk)
    test_data = dict((k,v) for k,v in data.iteritems() if k in tstk)
    train(train_data)
    p = classify(test_data)
    problist = []
    labellist = []
    for each in p['details'].keys():
        dat = p['details'][each]
